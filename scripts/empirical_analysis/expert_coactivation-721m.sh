#!/bin/bash
# Compute the expert coactivation for FLAME-MoE-721M-3.8B.

#SBATCH --job-name=expert-coactivation-721m
...
# process each layer inside each checkpoint for FLAME-MoE-721M-3.8B
find $SSD_MOUNT/actives -mindepth 2 -maxdepth 2 -type d | while read -r actives_path; do
    results_path=results/expert-coactivation/flame-moe-721m/$(basename $(dirname $actives_path))/$(basename $actives_path).pkl
