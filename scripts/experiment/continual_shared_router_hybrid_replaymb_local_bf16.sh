...
if [ "$MOE_JOINT_REPLAY_LM" = "1" ]; then
    ...
    # Fork-only: per-DP-rank replay micro-batch (0/unset = same as --micro-batch-size).
    if [ -n "${JOINT_REPLAY_MICRO_BATCH_SIZE:-}" ] && [ "${JOINT_REPLAY_MICRO_BATCH_SIZE}" != "0" ]; then
        JOINT_REPLAY_ARGS+=(--moe-joint-replay-micro-batch-size "$JOINT_REPLAY_MICRO_BATCH_SIZE")
    fi
