...
# Task B after Task A (local, experimental fp32 path): resume from a completed
# Task A checkpoint with the same expert count and continue training on Task B
# without freezing any experts or router parameters.

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$PROJECT_ROOT"

export RUN_ID="${RUN_ID:-stage-b-after-a-no-freeze-local-fp32-$(date -u +%Y%m%d-%H%M%S)}"
...
required_iters = int(os.environ.get("STAGE_A_REQUIRED_ITERS", "1"))
best = None
best_mtime = -1.0
...
if best is None:
    raise SystemExit(
        "ERROR: could not find a completed Task A fp32 run. "
...
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$LOCAL_WEIGHTS/continual-stage-B-after-A-no-freeze-local/$RUN_ID}"
export LOG_DIR="${LOG_DIR:-$TRAIN_WEIGHTS/logs}"
export RUN_LOG="${RUN_LOG:-$LOG_DIR/stage_b_after_a_no_freeze.log}"
...
export SECONDARY_PROBE_DATASET="${SECONDARY_PROBE_DATASET:-$LOCAL_DATASET/python-code-full/tokenized/EleutherAI/pythia-12b}"
...
if [ "$RESUME_ITERATION" != "latest" ]; then
    checkpoint_dir="$STAGE_A_WEIGHTS_DIR/iter_$(printf '%07d' "$RESUME_ITERATION")"
...
keep_re = re.compile(r"Stage B after A no-freeze run log:|Stage B after A no-freeze GPU log:|Stage B after A no-freeze metadata:|Stage B after A no-freeze source:|Stage B after A no-freeze complete|Stage B after A no-freeze run log saved at:|Stage B after A no-freeze GPU log saved at:|Stage B after A no-freeze metadata saved at:|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe ")
...
echo "Stage B after A no-freeze run log: $RUN_LOG"
echo "Stage B after A no-freeze GPU log: $GPU_LOG"
echo "Stage B after A no-freeze metadata: $RUN_METADATA"
echo "Stage B after A no-freeze source: $STAGE_A_WEIGHTS_DIR at iteration $RESUME_ITERATION"
echo "Stage B after A no-freeze output checkpoint: $TRAIN_WEIGHTS"
...
echo "Stage B after A no-freeze complete. Checkpoint at: $TRAIN_WEIGHTS"
echo "Stage B after A no-freeze run log saved at: $RUN_LOG"
echo "Stage B after A no-freeze GPU log saved at: $GPU_LOG"
echo "Stage B after A no-freeze metadata saved at: $RUN_METADATA"
