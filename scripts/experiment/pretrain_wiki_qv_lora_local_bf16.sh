...
export RUN_ID="${RUN_ID:-wiki-qv-lora-pretrain-local-bf16-$(date -u +%Y%m%d-%H%M%S)}"
...
export RUN_LOG="${RUN_LOG:-$LOG_DIR/wiki_qv_lora_pretrain.log}"
...
export SECONDARY_PROBE_DATASET="${SECONDARY_PROBE_DATASET:-}"
export SECONDARY_PROBE_NAME="${SECONDARY_PROBE_NAME:-secondary_probe}"
...
keep_re = re.compile(r"wiki qv lora|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe ")
...
echo "wiki qv lora run log: $RUN_LOG"
echo "wiki qv lora GPU log: $GPU_LOG"
echo "wiki qv lora metadata: $RUN_METADATA"
...
echo "wiki qv lora pretrain complete. Checkpoint at: $TRAIN_WEIGHTS"
echo "wiki qv lora run log saved at: $RUN_LOG"
