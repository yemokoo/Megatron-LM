...
# Stage A (local, experimental fp32 path): try a numerically conservative
# local-only launcher for 3090-class GPUs without changing the existing path.

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$PROJECT_ROOT"

export RUN_ID="${RUN_ID:-stage-a-local-fp32-$(date -u +%Y%m%d-%H%M%S)}"
...
# Keep the same model family by default. These defaults now target the
# validated 8x3090 fp32 path and can still be overridden by env vars.
...
export GLOBAL_BATCH_SIZE="${GLOBAL_BATCH_SIZE:-2048}"
...
export RUN_LOG="${RUN_LOG:-$LOG_DIR/stage_a.log}"
...
keep_re = re.compile(r"Stage A run log:|Stage A GPU log:|Stage A metadata:|Stage A local complete|Stage A run log saved at:|Stage A GPU log saved at:|Stage A metadata saved at:|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe ")
...
echo "Stage A run log: $RUN_LOG"
echo "Stage A GPU log: $GPU_LOG"
echo "Stage A metadata: $RUN_METADATA"
...
echo "Stage A local fp32 complete. Checkpoint at: $TRAIN_WEIGHTS"
echo "Stage A run log saved at: $RUN_LOG"
