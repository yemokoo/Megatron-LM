...
local_weights = Path(os.environ['LOCAL_WEIGHTS']) / 'wiki-qv-lora-pretrain-local'
...
for candidate in local_weights.iterdir() if local_weights.exists() else []:
    ...
    if mtime > best_mtime:
        ...
        best_mtime = mtime
if best is None:
    raise SystemExit('ERROR: could not find a completed wiki qv lora pretrain run. Set STAGE1_WEIGHTS_DIR explicitly.')
print(best)
PY
}
...
export RUN_ID="${RUN_ID:-code-from-wiki-qv-lora-expand-local-bf16-$(date -u +%Y%m%d-%H%M%S)}"
...
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$LOCAL_WEIGHTS/code-from-wiki-qv-lora-expand-local/$RUN_ID}"
export LOG_DIR="${LOG_DIR:-$TRAIN_WEIGHTS/logs}"
export RUN_LOG="${RUN_LOG:-$LOG_DIR/code_from_wiki_qv_lora_expand.log}"
...
keep_re = re.compile(r"code from wiki qv lora|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe |Expanded attention LoRA")
...
echo "code-from-wiki qv lora run log: $RUN_LOG"
echo "code-from-wiki qv lora GPU log: $GPU_LOG"
echo "code-from-wiki qv lora metadata: $RUN_METADATA"
echo "code-from-wiki qv lora source checkpoint: $STAGE1_WEIGHTS_DIR"
...
metadata = {
    'stage': 'code_from_wiki_qv_lora_expand',
...
echo "code-from-wiki qv lora continual training complete. Checkpoint at: $TRAIN_WEIGHTS"
echo "code-from-wiki qv lora run log saved at: $RUN_LOG"
