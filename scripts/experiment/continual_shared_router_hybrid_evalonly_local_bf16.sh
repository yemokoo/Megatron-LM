...
# Fork-only: eval-only 패스용. --skip-train 이면 megatron 이 학습 루프에 진입하지 않고
# (training.py) 초기 프로브만 돌린 뒤 종료하며, 체크포인트 저장 경로를 아예 타지 않는다.
SKIP_TRAIN_ARGS=()
if [ "${SKIP_TRAIN:-0}" = "1" ]; then
    SKIP_TRAIN_ARGS+=(--skip-train)
fi
...
if [ "${SKIP_TRAIN:-0}" != "1" ] && [ "$SSD_TARGET_WEIGHTS" != "$TRAIN_WEIGHTS" ]; then
