...
export RUN_ID="${RUN_ID:-g2-top4-e8to16-ffn352-r256-wiki-to-code-shared-router-qkvo-all-experts-router-mha-a100-bf16-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
...
echo "[CONFIG] G2 shared-router all experts+router code training"
echo "[CONFIG] code steps=${TRAIN_ITERS}, mb=${MICRO_BATCH_SIZE}, gbs=${GLOBAL_BATCH_SIZE}"
echo "[CONFIG] trainable: all FFN experts, all attention LoRA experts, all router rows"
...
env \
    ...
    WANDB_EXP_NAME="${WANDB_EXP_NAME:-G2 shared-router all experts+router - wiki to code}" \
    ...
    bash -lc 'if [ "$USE_GUARD" = "1" ]; then exec "$SCRIPT_DIR/run_guarded_training.sh" bash "$SCRIPT_DIR/code_from_wiki_e6_fullrank_qkvo_all_experts_router_mha_a100_bf16.sh"; else exec bash "$SCRIPT_DIR/code_from_wiki_e6_fullrank_qkvo_all_experts_router_mha_a100_bf16.sh"; fi'

echo "[DONE] G2 shared-router all experts+router code training $(date)"
