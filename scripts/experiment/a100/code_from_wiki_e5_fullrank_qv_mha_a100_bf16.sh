...
unset ATTN_LORA_INCLUDE_PROJ
export LOCAL_BASE="${LOCAL_BASE:-$PROJECT_ROOT/.local}"
export LOCAL_WEIGHTS="${LOCAL_WEIGHTS:-$LOCAL_BASE/weights}"
export STAGE1_SUBDIR="${STAGE1_SUBDIR:-a100/mha/wiki-shared-router-hybrid-pretrain-local}"
export RUN_ID="${RUN_ID:-e5-wiki-to-code-ffn-attn-lora-qv-r${ATTN_LORA_RANK}-single-router-moe-mha-a100-bf16-$(date -u +%Y%m%d-%H%M%S)}"
...
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-E5 - FullRank QV LoRA Expert - wiki to code}"
