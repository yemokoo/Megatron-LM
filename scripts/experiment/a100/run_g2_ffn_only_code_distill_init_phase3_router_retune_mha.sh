...
usage() {
    cat >&2 <<'EOF'
usage: run_g2_ffn_only_code_distill_init_phase3_router_retune_mha.sh <mode|all>

Stage C of the FFN-only pre-Code expert-init experiment: router-only retuning on
wiki+code mixed data (RETUNE_ITERS steps) for each Stage B checkpoint produced by
run_g2_ffn_only_code_from_distill_init_mha.sh.
...
before retuning, so the Stage B checkpoints are not modified. Only the MoE router
(all rows) trains; experts and everything else stay frozen. Data = wiki + code
(equal blend). This mirrors run_g2matched_phase3_router_only_retune_mha.sh but
sources the distill-init lineage instead of the random-init baseline.
EOF
}
...
# FFN-only MoE, router-only retune settings (match g2matched phase3).
...
export STAGE_B_MB="${STAGE_B_MB:-96}"
export STAGE_B_ITERS="${STAGE_B_ITERS:-1800}"
export STAGE_B_ROOT="${STAGE_B_ROOT:-$G2_ROOT/code/from_distill_init}"
export PHASE3_ROOT="${PHASE3_ROOT:-$G2_ROOT/code/from_distill_init_phase3}"
mkdir -p "$PHASE3_ROOT"
...
ensure_copy() {
    ...
    {
        echo "phase=3"
        echo "purpose=ffn-only distill-init Stage C router-only retuning copy"
...
run_one() {
    ...
    local source_run_id="g2-ffn-only-e8to16-code-from-distill-init-${safe_mode}-mha-a100-bf16-mb${STAGE_B_MB}-${STAGE_B_ITERS}"
    ...
    if [ ! -f "$src/latest_checkpointed_iteration.txt" ]; then
        ...
        echo "        Run Stage B first: run_g2_ffn_only_code_from_distill_init_mha.sh $mode" >&2
        exit 1
    fi

    local run_id="g2-ffn-only-code-from-distill-init-${safe_mode}-phase3-router-retune-wikicode-mb${MICRO_BATCH_SIZE}-${RETUNE_ITERS}"
    local exp_name="G2 FFN-only - ${mode} distill-init Stage C router-only retune wiki+code"
    local weights="$PHASE3_ROOT/$run_id"
    local port

    case "$mode" in
        logits) port="${MASTER_PORT_LOGITS:-29781}" ;;
        logits_hidden) port="${MASTER_PORT_LOGITS_HIDDEN:-29782}" ;;
        logits_hidden_router) port="${MASTER_PORT_LOGITS_HIDDEN_ROUTER:-29783}" ;;
    esac
    ...
    echo "[CONFIG] G2 FFN-only distill-init Stage C router-only retune (mode=$mode)"
    ...
    echo "[CONFIG] data=wiki train + code train (equal blend), trainable=MoE router only"
