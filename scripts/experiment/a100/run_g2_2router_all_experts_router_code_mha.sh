...
export RUN_ID="${RUN_ID:-g2-2router-top4-e8to16-ffn352-r256-wiki-to-code-independent-router-qkvo-all-experts-router-mha-a100-bf16-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
export STAGE1_WEIGHTS_DIR="${STAGE1_WEIGHTS_DIR:-$BASE_STAGE_DIR/wiki/g2-2router-top4-e8-ffn352-r256-wiki-independent-router-qkvo-mha-a100-bf16-mb72-1800}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$BASE_STAGE_DIR/code/phase1/$RUN_ID}"
...
if ! is_completed "$STAGE1_WEIGHTS_DIR"; then
    echo "[ERROR] missing completed G2-2router wiki checkpoint: $STAGE1_WEIGHTS_DIR" >&2
    echo "[HINT] run scripts/experiment/a100/run_g2_2router_wiki_mha.sh first." >&2
    exit 1
fi
...
echo "[CONFIG] G2-2router Exp1 code training"
echo "[CONFIG] code steps=${TRAIN_ITERS}, mb=${MICRO_BATCH_SIZE}, gbs=${GLOBAL_BATCH_SIZE}"
echo "[CONFIG] trainable: all FFN experts, all attention LoRA experts, attn routers, FFN routers"
echo "[CONFIG] frozen: shared dense trunk parameters"
...
env \
    ...
    WANDB_EXP_NAME="${WANDB_EXP_NAME:-G2-2router Exp1 all experts+routers - wiki to code}" \
    ...
    bash -lc 'if [ "$USE_GUARD" = "1" ]; then exec "$SCRIPT_DIR/run_guarded_training.sh" bash "$PROJECT_ROOT/scripts/experiment/continual_code_from_wiki_shared_router_hybrid_expand_local_bf16.sh"; else exec bash "$PROJECT_ROOT/scripts/experiment/continual_code_from_wiki_shared_router_hybrid_expand_local_bf16.sh"; fi'

echo "[DONE] G2-2router Exp1 code training $(date)"
