...
if [ "$(latest "$CODE_OUT")" != 1800 ]; then
  ...
  MICRO_BATCH_SIZE="${CODE_MB:-96}" MASTER_PORT=29940 \
...
if [ "$(latest "$CODE_LPR_OUT")" != 2160 ]; then
  ...
  MICRO_BATCH_SIZE="${CODE_LPR_MB:-96}" MASTER_PORT=29941 \
...
if [ "$(latest "$CONV_OUT")" != 1800 ]; then
  ...
  FFN_MICRO_BATCH_SIZE="${CONV_MB:-64}" SOURCE_LOGICAL_STEP=2160 FFN_MASTER_PORT=29942 \
...
if [ "$(latest "$CONV_LPR_OUT")" != 2160 ]; then
  ...
  MICRO_BATCH_SIZE="${CONV_LPR_MB:-64}" MASTER_PORT=29943 \
