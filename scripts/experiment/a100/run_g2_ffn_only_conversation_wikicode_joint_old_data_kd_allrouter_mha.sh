...
: "${SOURCE_WEIGHTS_DIR:?set SOURCE_WEIGHTS_DIR to the expanded 24E Conversation KD-init checkpoint}"
: "${OLD_MODEL_KL_WEIGHTS_DIR:?set OLD_MODEL_KL_WEIGHTS_DIR to the frozen 16E Code teacher checkpoint}"
...
export STAGE_DIR_NAME=a100/mha/g2-checkpoints/conversation/joint_old_data_kd
export RUN_ID="${RUN_ID:-g2-ffn-only-conversation-lm-wikicode-kd-allrouter-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$G2_ROOT/conversation/joint_old_data_kd/$RUN_ID}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-G2 FFN-only Conversation LM + WikiCode old-data KD all-router}"

echo "[CONFIG] primary=Conversation LM; replay=equal Wiki+Code teacher-logits KD only"
