...
export ATTN_FULL_RANK_LORA_TARGETS="${ATTN_FULL_RANK_LORA_TARGETS:-qvo}"
export LOCAL_BASE="${LOCAL_BASE:-$PROJECT_ROOT/.local}"
export LOCAL_WEIGHTS="${LOCAL_WEIGHTS:-$LOCAL_BASE/weights}"
export SOURCE_RUN_SUBDIR="${SOURCE_RUN_SUBDIR:-a100/mha/wiki-a-moe-bf16}"
export STAGE_DIR_NAME="${STAGE_DIR_NAME:-a100/mha/f-attn-qvo-full-rank-lora-bf16}"
export RUN_ID="${RUN_ID:-fqvo-wiki-to-code-ffn-moe-unfreeze-attn-full-rank-lora-mha-a100-bf16-$(date -u +%Y%m%d-%H%M%S)}"
export WANDB_PROJECT="${WANDB_PROJECT:-flame-continual-top2-qv-lora}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-F-QVO - Wiki to Code FFN MoE Unfreeze + Attn Full-Rank LoRA MHA}"
