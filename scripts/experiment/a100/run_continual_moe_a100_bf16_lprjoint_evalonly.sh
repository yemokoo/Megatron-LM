...
# ZeRO-1 스타일 옵티마이저 상태 샤딩.  이 경로는 --no-load-optim/--no-save-optim 이
# 항상 켜져 있어 체크포인트 호환성 문제가 없다.  Lifelong-MoE 처럼 가중치+옵티마이저가
# 메모리를 지배하는 설정에서 micro batch 를 키울 여지를 만든다.
if [ "${USE_DISTRIBUTED_OPTIMIZER:-0}" = "1" ]; then
    SAVE_ARGS+=(--use-distributed-optimizer)
    [ "${OVERLAP_GRAD_REDUCE:-0}" = "1" ] && SAVE_ARGS+=(--overlap-grad-reduce)
    [ "${OVERLAP_PARAM_GATHER:-0}" = "1" ] && SAVE_ARGS+=(--overlap-param-gather)
fi
