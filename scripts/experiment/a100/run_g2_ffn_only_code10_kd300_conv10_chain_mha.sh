...
CODE_CHUNK="${CODE_CHUNK:-10}"
CODE_ROUTER_CHUNK="${CODE_ROUTER_CHUNK:-10}"
KD_STEPS="${KD_STEPS:-300}"
CONV_STEPS="${CONV_STEPS:-1800}"
CONV_CHUNK="${CONV_CHUNK:-10}"
CONV_ROUTER_CHUNK="${CONV_ROUTER_CHUNK:-10}"

CODE_MB="${CODE_MB:-48}"
KD_MB="${KD_MB:-36}"
CONV_MB="${CONV_MB:-48}"

if [ "$CODE_CHUNK" -le 0 ] || [ "$CODE_ROUTER_CHUNK" -le 0 ] ||
   [ "$CONV_CHUNK" -le 0 ] || [ "$CONV_ROUTER_CHUNK" -le 0 ]; then
    echo "[ERROR] all interleave chunk sizes must be positive" >&2
    exit 1
fi

G2_ROOT="${G2_ROOT:-$PROJECT_ROOT/.local/weights/a100/mha/g2-checkpoints}"
LOG_DIR="${LOG_DIR:-$PROJECT_ROOT/.local/logs/g2_code10_kd300_conv10_chain}"
mkdir -p "$LOG_DIR"

CODE_CHUNKS=$(((CODE_STEPS + CODE_CHUNK - 1) / CODE_CHUNK))
CODE_ROUTER_STEPS=$((CODE_CHUNKS * CODE_ROUTER_CHUNK))
CODE_EXPECTED_STEP=$((CODE_STEPS + CODE_ROUTER_STEPS))
CODE_RUN_ID="${CODE_RUN_ID:-g2-ffn-only-interleaved-code${CODE_CHUNK}-router${CODE_ROUTER_CHUNK}-from-logits-wiki-distill-mb${CODE_MB}-code${CODE_STEPS}}"
...
CONV_CHUNKS=$(((CONV_STEPS + CONV_CHUNK - 1) / CONV_CHUNK))
CONV_ROUTER_STEPS=$((CONV_CHUNKS * CONV_ROUTER_CHUNK))
CONV_EXPECTED_STEP=$((CONV_STEPS + CONV_ROUTER_STEPS))
CONV_RUN_ID="${CONV_RUN_ID:-g2-ffn-only-interleaved-conv${CONV_CHUNK}-router${CONV_ROUTER_CHUNK}-wikicodeconv-from-kd-mb${CONV_MB}-conv${CONV_STEPS}}"
...
echo "[CHAIN] Code10/router10 -> Wiki+Code output KD300 -> Conversation10/router10"
echo "[CHAIN] GPUs=$CUDA_VISIBLE_DEVICES nproc=$NPROC_PER_NODE"
echo "[CHAIN] Stage 1: Code=$CODE_STEPS, router=$CODE_ROUTER_STEPS, chunks=$CODE_CHUNKS, output iteration=$CODE_EXPECTED_STEP"
echo "[CHAIN] Stage 2: Wiki+Code logits KD=$KD_STEPS, 16->24 experts"
echo "[CHAIN] Stage 3: Conversation=$CONV_STEPS, Wiki+Code+Conversation router=$CONV_ROUTER_STEPS, chunks=$CONV_CHUNKS"
...
run_stage \
    "Stage 1 / Code${CODE_CHUNK}-router${CODE_ROUTER_CHUNK}" \
    "$CODE_WEIGHTS" \
    "$CODE_EXPECTED_STEP" \
    "$LOG_DIR/stage1_code10_router10.log" \
    env \
        TOKENIZER_MODEL="$TOKENIZER_MODEL" \
        MICRO_BATCH_SIZE="$CODE_MB" \
        MOE_INTERLEAVE_CODE_TOTAL_STEPS="$CODE_STEPS" \
        MOE_INTERLEAVE_CODE_STEPS="$CODE_CHUNK" \
        MOE_INTERLEAVE_ROUTER_STEPS="$CODE_ROUTER_CHUNK" \
        RUN_ID="$CODE_RUN_ID" \
        TRAIN_WEIGHTS="$CODE_WEIGHTS" \
        MASTER_PORT="${CODE_MASTER_PORT:-29971}" \
        bash "$CHAIN_SCRIPT_DIR/run_g2_ffn_only_interleaved_code_router_from_distill_init_mha.sh" logits

run_stage \
    "Stage 2 / Wiki+Code output-logits KD 16->24" \
    "$KD_WEIGHTS" \
    "$KD_EXPECTED_STEP" \
    "$LOG_DIR/stage2_wikicode_kd300.log" \
    env \
        TOKENIZER_MODEL="$TOKENIZER_MODEL" \
