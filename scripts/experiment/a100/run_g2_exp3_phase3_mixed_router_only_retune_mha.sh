...
export RUN_ID="${RUN_ID:-g2-exp3-phase2-router-only-retune-wikicode-from-routerkd-kl10p0-top4plus-allcode-all-router-no-reinit-mb${MICRO_BATCH_SIZE}-${RETUNE_ITERS}}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$G2_ROOT/code/phase3/$RUN_ID}"
export MASTER_PORT="${MASTER_PORT:-29786}"
...
if [ ! -d "$TRAIN_WEIGHTS" ]; then
    echo "[COPY] source -> mixed all-router retune copy"
    ...
    {
        echo "phase=exp3_phase2"
        echo "purpose=wiki+code router-only retuning copy"
        ...
        echo "data=wiki train + code train"
        echo "forward=ordinary top${MOE_ROUTER_TOPK} over all ${NUM_EXPERTS} experts"
        echo "trainable=all shared-router rows only"
        echo "frozen=all experts, attention LoRA experts, dense trunk, embeddings, output"
        echo "loss=wiki+code mixed LM loss"
        echo "copied_at=$(date -Iseconds)"
    } > "$TRAIN_WEIGHTS/PHASE3_SOURCE.txt"
else
    echo "[COPY] using existing mixed all-router retune copy: $TRAIN_WEIGHTS"
fi

echo "[CONFIG] G2 Exp3 Phase 2 mixed all-router retune"
echo "[CONFIG] source_step=${src_step}, target_step=${target_step}, retune_iters=${RETUNE_ITERS}"
echo "[CONFIG] data=wiki train + code train"
echo "[CONFIG] forward=normal top${MOE_ROUTER_TOPK} over wiki+code experts; no all-code union"
echo "[CONFIG] trainable=all shared-router rows only"
echo "[CONFIG] frozen=all experts + dense/shared params"
...
env \
    ...
    WANDB_EXP_NAME="${WANDB_EXP_NAME:-G2 - exp3 Phase 2 mixed all-router retune wiki+code}" \
    WANDB_RUN_ID="$RUN_ID" \
