#!/bin/bash
# =============================================================================
# G2 FFN-only pre-Code expert-init distillation — 9-run sequential driver
#
#   Stage A (x3): expand 8->16 + KL distill on WIKI      (mb48, teacher resident)
#   Stage B (x3): code training from the distill-init    (mb96, no teacher)
...
#
# Launch it detached with:  bash launch.sh   (see that script / README.md)
# =============================================================================
set -euo pipefail
...
MB_BC="${MB_BC:-96}"      # Stage B/C micro-batch
...
LOGDIR="${LOGDIR:-$PROJECT_ROOT/.local/logs/g2_ffn_only_distill_init}"
mkdir -p "$LOGDIR"

safe(){ echo "${1//_/-}"; }
A_dir(){ echo "$G2_ROOT/code/expansion_distill_init/g2-ffn-only-e8to16-code-expert-init-$(safe "$1")-wiki-distill-mha-a100-bf16-mb${MB_A}-${ITERS}"; }
B_dir(){ echo "$G2_ROOT/code/from_distill_init/g2-ffn-only-e8to16-code-from-distill-init-$(safe "$1")-mha-a100-bf16-mb${MB_BC}-${ITERS}"; }
C_dir(){ echo "$G2_ROOT/code/from_distill_init_phase3/g2-ffn-only-code-from-distill-init-$(safe "$1")-phase3-router-retune-wikicode-mb${MB_BC}-${RETUNE}"; }
...
for m in $MODES; do
  ...
  MICRO_BATCH_SIZE="$MB_A" MASTER_PORT=29621 \
    bash "$A100_DIR/run_g2_ffn_only_code_expert_distill_init_mha.sh" "$m" 2>&1 | tee "$LOGDIR/A_$m.log"
done
...
for m in $MODES; do
  ...
  MICRO_BATCH_SIZE="$MB_BC" DISTILL_SOURCE_MB="$MB_A" MASTER_PORT=29622 \
    bash "$A100_DIR/run_g2_ffn_only_code_from_distill_init_mha.sh" "$m" 2>&1 | tee "$LOGDIR/B_$m.log"
done
...
for m in $MODES; do
  ...
  MICRO_BATCH_SIZE="$MB_BC" \
    bash "$A100_DIR/run_g2_ffn_only_code_distill_init_phase3_router_retune_mha.sh" "$m" 2>&1 | tee "$LOGDIR/C_$m.log"
done
