...
WIKI_MICRO_BATCH_SIZE="${WIKI_MICRO_BATCH_SIZE:-128}"
WIKI_RUN_ID="${WIKI_RUN_ID:-g2matched-top4-e8-ffn352-wiki-ffn-moe-mha-a100-bf16-mb${WIKI_MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
WIKI_WEIGHTS="${SOURCE_WEIGHTS_DIR:-$BASE_WEIGHTS_DIR/wiki-a-moe-g2matched-bf16/$WIKI_RUN_ID}"

ATTN_FREEZE_RUN_ID="${ATTN_FREEZE_RUN_ID:-g2matched-top4-e8to16-ffn352-wiki-to-code-ffn-moe-attn-freeze-mha-a100-bf16-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
ATTN_LORA_RUN_ID="${ATTN_LORA_RUN_ID:-g2matched-top4-e8to16-ffn352-wiki-to-code-ffn-moe-attn-fullrank-qkvo-r${ATTN_FULL_RANK_LORA_RANK}-mha-a100-bf16-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
...
if ! is_completed "$WIKI_WEIGHTS"; then
    echo "ERROR: completed wiki source checkpoint not found: $WIKI_WEIGHTS" >&2
    echo "Set SOURCE_WEIGHTS_DIR=/path/to/wiki/run or WIKI_RUN_ID=..." >&2
    exit 1
fi

echo "[CONFIG] G2-matched FFN code baselines"
echo "[CONFIG] source wiki=$WIKI_WEIGHTS"
echo "[CONFIG] common: topk=4, source experts=8, target experts=16, moe_ffn_hidden=352"
echo "[CONFIG] code micro_batch_size=$MICRO_BATCH_SIZE, wandb_mode=$WANDB_MODE"
