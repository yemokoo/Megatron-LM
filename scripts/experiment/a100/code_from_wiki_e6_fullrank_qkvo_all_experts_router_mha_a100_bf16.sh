...
export SHARED_ROUTER_HYBRID_TRAIN_ALL_EXPERTS_AND_ROUTER=1
export SHARED_ROUTER_HYBRID_TRAIN_ALL_ROUTER_ROWS=1
export RUN_ID="${RUN_ID:-e6-wiki-to-code-ffn-attn-fullrank-qkvo-all-experts-router-single-router-moe-mha-a100-bf16-$(date -u +%Y%m%d-%H%M%S)}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$LOCAL_WEIGHTS/a100/mha/code-from-wiki-shared-router-hybrid-all-experts-router-local/$RUN_ID}"
export WANDB_PROJECT="${WANDB_PROJECT:-flame-continual-top2-qv-lora}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-E6 - Wiki to Code FFN+Attn Full-Rank QKVO All Experts+Router Single Router MoE MHA}"
