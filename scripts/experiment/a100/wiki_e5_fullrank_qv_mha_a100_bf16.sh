...
export RUN_ID="${RUN_ID:-e5-wiki-ffn-attn-lora-qv-r${ATTN_LORA_RANK}-single-router-moe-mha-a100-bf16-$(date -u +%Y%m%d-%H%M%S)}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$LOCAL_WEIGHTS/a100/mha/wiki-shared-router-hybrid-pretrain-local/$RUN_ID}"
export WANDB_PROJECT="${WANDB_PROJECT:-flame-continual-top2-qv-lora}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-E5 - FullRank QV LoRA Expert - wiki}"
