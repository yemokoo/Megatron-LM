#!/bin/bash
# LPR variant of phase3_router_only_retune_shared_router_hybrid_mixed_local_bf16.sh:
# adds the MOE_LPR_* -> --moe-lpr-* plumbing and the equal_dataset mix used by
# the FFN-only MoE-LPR retune.  Kept as a separate file so the stock script is
# untouched.  Used by run_moe_lpr_hybrid_from_hf_e8.sh.
set -euo pipefail
...
# Optional supervised task-group LPR loss (MoE-LPR router retune); off unless set.
...
if [ -n "$TRAIN_DATASET_CONVERSATION" ]; then
    echo "phase3 router-only mixed conversation train dataset: $TRAIN_DATASET_CONVERSATION"
fi
...
SAVE_ARGS=(
    ...
    --no-save-optim   # LPR branch: optimizer sharding validation fails on partially-trainable experts; next stage loads with --no-load-optim
)
...
elif [ "$MIXED_DATA_WEIGHT_MODE" = "equal_dataset" ]; then
    # same mix the FFN-only MoE-LPR retune uses: each dataset gets equal total weight
