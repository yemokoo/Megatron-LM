...
export STAGE_DIR_NAME=a100/mha/g2-checkpoints/conversation/joint_old_data_hidden_kl
export RUN_ID="${RUN_ID:-g2-conv-lm-wikicode-hidden-kl-allrouter-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$G2_ROOT/conversation/joint_old_data_hidden_kl/$RUN_ID}"
export WANDB_EXP_NAME="${WANDB_EXP_NAME:-G2 Conversation LM + WikiCode layer-hidden KL all-router}"

echo "[CONFIG] primary=Conversation LM; replay=Wiki+Code 1:1 layer-output hidden KL"
