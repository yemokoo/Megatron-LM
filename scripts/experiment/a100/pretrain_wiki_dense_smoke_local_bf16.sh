...
export RUN_ID="${RUN_ID:-wiki-dense-pretrain-local-bf16-$(date -u +%Y%m%d-%H%M%S)}"
...
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$LOCAL_WEIGHTS/wiki-dense-pretrain-local/$RUN_ID}"
export LOG_DIR="${LOG_DIR:-$TRAIN_WEIGHTS/logs}"
export RUN_LOG="${RUN_LOG:-$LOG_DIR/wiki_dense_pretrain.log}"
...
SMOKE_ARGS=()
if [ "${SMOKE:-0}" = "1" ]; then
    SMOKE_ARGS=(--skip-train-end-save)
fi
...
keep_re = re.compile(r"wiki dense|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe ")
...
echo "wiki dense run log: $RUN_LOG"
