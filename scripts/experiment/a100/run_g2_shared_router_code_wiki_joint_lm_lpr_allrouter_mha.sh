...
# LPR-joint variant: MoE-LPR's supervised old-expert group loss is added INSIDE
# the wiki replay pass of this 1-phase run (shared-router hybrid: FFN + QKVO
# attention experts).  JOINT_REPLAY_LPR_COEFF=0 reproduces the stock run.
...
export RUN_ID="${RUN_ID:-g2-shared-router-code-wiki-joint-lm-lpr${JOINT_REPLAY_LPR_COEFF}-allrouter-${SAFE_MODE}-init-mb${MICRO_BATCH_SIZE}-${TRAIN_ITERS}}"
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:-$G2_ROOT/code/shared_router_joint_lm_replay_lpr/$RUN_ID}"
