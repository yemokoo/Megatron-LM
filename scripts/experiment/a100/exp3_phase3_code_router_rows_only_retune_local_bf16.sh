...
export RUN_ID="${RUN_ID:-exp3-phase3-code-router-rows-only-retune-bf16-$(date -u +%Y%m%d-%H%M%S)}"
...
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:?TRAIN_WEIGHTS must point to the copied Exp3 Phase 3 checkpoint}"
...
if [ ! -f "$TRAIN_WEIGHTS/latest_checkpointed_iteration.txt" ]; then
    echo "ERROR: Exp3 Phase 3 checkpoint copy is missing latest_checkpointed_iteration.txt: $TRAIN_WEIGHTS" >&2
    exit 1
fi
...
# Exp3 Phase 3 intentionally returns to ordinary top-k routing.
...
mkdir -p "$SSD_CODE_TRAIN" "$SSD_TARGET_WEIGHTS" "$TRAIN_WEIGHTS" "$LOG_DIR"
exec > >(
...
keep_re = re.compile(r"exp3 phase3|code-router|router-only|shared-router hybrid|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe ")
...
echo "exp3 phase3 code-router-row-only run log: $RUN_LOG"
echo "exp3 phase3 code-router-row-only gpu log: $GPU_LOG"
echo "exp3 phase3 code-router-row-only metadata: $RUN_METADATA"
...
metadata = {
    'stage': 'exp3_phase3_code_router_rows_only_retune',
    ...
    'code_train': code,
    'trainable': 'shared-router code rows only',
