...
if [ "$SMOKE" = 1 ]; then
  ...
  # Fork-only: MoE-LPR 구조(task -> LPR 리튠)를 hybrid 아키텍처로 돌리기 위해 env 로 개방.
  IT_P1="${IT_P1:-1800}"; IT_RFT_CODE="${IT_RFT_CODE:-720}"; IT_RFT_CONV="${IT_RFT_CONV:-1080}"; IT_KD="${IT_KD:-600}"; export GLOBAL_BATCH_SIZE=2304
...
if [ "$(latest "$CODE_RFT")" != "$RFT_CODE_STEP" ]; then
  ...
  if [ "$VARIANT" = hyb ]; then
    env "${rft_common[@]}" MOE_LPR_LOSS_COEFF="${MOE_LPR_LOSS_COEFF:-0}" bash "$A/phase3_router_only_retune_shared_router_hybrid_lpr_local_bf16.sh"
  else
