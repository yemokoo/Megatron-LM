...
export RUN_ID="${RUN_ID:-phase3-router-only-retune-moe-mixed-bf16-$(date -u +%Y%m%d-%H%M%S)}"
...
export TRAIN_WEIGHTS="${TRAIN_WEIGHTS:?TRAIN_WEIGHTS must point to the router-retuning checkpoint copy}"
export LOG_DIR="${LOG_DIR:-$TRAIN_WEIGHTS/logs}"
export RUN_LOG="${RUN_LOG:-$LOG_DIR/phase3_moe_router_run.log}"
export GPU_LOG="${GPU_LOG:-$LOG_DIR/phase3_moe_router_gpu.log}"
export RUN_METADATA="${RUN_METADATA:-$LOG_DIR/phase3_moe_router_metadata.json}"
...
if [ ! -f "$TRAIN_WEIGHTS/latest_checkpointed_iteration.txt" ]; then
    echo "ERROR: checkpoint copy is missing latest_checkpointed_iteration.txt: $TRAIN_WEIGHTS" >&2
    exit 1
fi
...
export SOURCE_STEP="${SOURCE_STEP:-$(read_source_step)}"
