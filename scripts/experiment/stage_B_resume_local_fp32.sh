...
# Resume Task B fp32 training from a specific saved Task B checkpoint step.
# This is separate from stage_B_local_fp32.sh, which performs the initial
# Stage A -> Stage B expansion.

PROJECT_ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)"
cd "$PROJECT_ROOT"

export RUN_ID="${RUN_ID:-stage-b-resume-local-fp32-$(date -u +%Y%m%d-%H%M%S)}"
...
export RESUME_ITERATION="${RESUME_ITERATION:-1200}"
...
export TRAIN_ITERS="${TRAIN_ITERS:-9796}"
...
export RUN_LOG="${RUN_LOG:-$LOG_DIR/stage_b_resume.log}"
...
export PROBE_STEP_OFFSET="${PROBE_STEP_OFFSET:-1874}"
...
checkpoint_dir="${SOURCE_B_WEIGHTS_DIR}/iter_$(printf '%07d' "$RESUME_ITERATION")"
...
keep_re = re.compile(r"Stage B resume run log:|Stage B resume GPU log:|Stage B resume metadata:|Stage B resume source:|Stage B resume complete|Stage B resume run log saved at:|Stage B resume GPU log saved at:|Stage B resume metadata saved at:|ERROR:|Traceback|failed \(exitcode|checkpoint at|probe ")
...
echo "Stage B resume run log: $RUN_LOG"
echo "Stage B resume GPU log: $GPU_LOG"
echo "Stage B resume metadata: $RUN_METADATA"
echo "Stage B resume source: $SOURCE_B_WEIGHTS_DIR at iteration $RESUME_ITERATION"
echo "Stage B resume output checkpoint: $TRAIN_WEIGHTS"
...
rsync -rlptD \
    ...
    "$SOURCE_B_WEIGHTS_DIR/" "$SSD_WEIGHTS/"
...
metadata = {
    ...
    "source_b_weights_dir": os.environ["SOURCE_B_WEIGHTS_DIR"],
    "resume_iteration": int(os.environ["RESUME_ITERATION"]),
...
echo "Stage B resume complete. Checkpoint at: $TRAIN_WEIGHTS"
echo "Stage B resume run log saved at: $RUN_LOG"
echo "Stage B resume GPU log saved at: $GPU_LOG"
echo "Stage B resume metadata saved at: $RUN_METADATA"
