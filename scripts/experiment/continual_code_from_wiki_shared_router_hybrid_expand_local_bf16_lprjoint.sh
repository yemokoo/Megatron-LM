...
if [ "$MOE_JOINT_REPLAY_LM" = "1" ]; then
    ...
    # 옛 데이터 예산.  기본 0 은 스텝마다 리플레이 글로벌 배치 1개(=1:1).
    # MOE_JOINT_REPLAY_TOTAL_SAMPLES 를 주면 전체 학습 동안 그만큼만 소비한다.
    if [ "${MOE_JOINT_REPLAY_TOTAL_SAMPLES:-0}" -gt 0 ]; then
        JOINT_REPLAY_ARGS+=(
            --moe-joint-replay-total-samples "$MOE_JOINT_REPLAY_TOTAL_SAMPLES"
            --moe-joint-replay-micro-batch-size "${MOE_JOINT_REPLAY_MICRO_BATCH_SIZE:-0}"
        )
    fi
fi
...
torchrun \
    ...
    "Megatron-LM/${TRAIN_ENTRY:-pretrain_gpt.py}" \
    "${MODEL_ARGS[@]}" \
