...
NE="${NE:-16}"; SRC="${SRC:-8}"; GPU="${GPU:-0}"; PORT="${PORT:-$((42200 + GPU))}"
...
CODE_PROBE=/data2/seonghyeonnoh/LLM-continual-learning-data/flamedata2.data2-verified-backup/code/test/test_text_document
...
CUDA_VISIBLE_DEVICES="$GPU" torchrun --nproc_per_node 1 --master_addr 127.0.0.1 --master_port "$PORT" \
  scripts/analysis/eod_context_probe.py "${MODEL_ARGS[@]}" \
  ...
  --probe-out "$OUT/eod_context_probe.json" --probe-wiki "$WIKI_PROBE" --probe-code "$CODE_PROBE" \
  2>&1 | tee "$OUT/eod_probe.log" | grep -E "^\[eod\]|Error|Traceback|error:" || true
[ -s "$OUT/eod_context_probe.json" ] && echo "[eod] DONE" || { echo "[eod] FAILED (see $OUT/eod_probe.log)"; exit 1; }
