...
export OUTPUT_ROOT="${OUTPUT_ROOT:-$LOCAL_WEIGHTS/mask-extra-router-suite}"
...
run_eval "baseline_a" "$BASELINE_A_LOAD" "$TASK_A_DATASET" "$OUTPUT_ROOT/baseline_a.json" "" "$MASTER_PORT"
run_eval "baseline_b" "$BASELINE_B_LOAD" "$TASK_B_DATASET" "$OUTPUT_ROOT/baseline_b.json" "" "$((MASTER_PORT + 1))"

run_eval "a_to_b" "$A_TO_B_LOAD" "$TASK_A_DATASET" "$OUTPUT_ROOT/a_to_b.json" "" "$((MASTER_PORT + 2))"
run_eval "a_to_b_masked" "$A_TO_B_LOAD" "$TASK_A_DATASET" "$OUTPUT_ROOT/a_to_b_masked.json" "$SOURCE_NUM_EXPERTS" "$((MASTER_PORT + 3))"

run_eval "a_to_b_new_only" "$A_TO_B_NEW_ONLY_LOAD" "$TASK_A_DATASET" "$OUTPUT_ROOT/a_to_b_new_only.json" "" "$((MASTER_PORT + 4))"
run_eval "a_to_b_new_only_masked" "$A_TO_B_NEW_ONLY_LOAD" "$TASK_A_DATASET" "$OUTPUT_ROOT/a_to_b_new_only_masked.json" "$SOURCE_NUM_EXPERTS" "$((MASTER_PORT + 5))"

run_eval "b_to_a" "$B_TO_A_LOAD" "$TASK_B_DATASET" "$OUTPUT_ROOT/b_to_a.json" "" "$((MASTER_PORT + 6))"
run_eval "b_to_a_masked" "$B_TO_A_LOAD" "$TASK_B_DATASET" "$OUTPUT_ROOT/b_to_a_masked.json" "$SOURCE_NUM_EXPERTS" "$((MASTER_PORT + 7))"

run_eval "b_to_a_new_only" "$B_TO_A_NEW_ONLY_LOAD" "$TASK_B_DATASET" "$OUTPUT_ROOT/b_to_a_new_only.json" "" "$((MASTER_PORT + 8))"
run_eval "b_to_a_new_only_masked" "$B_TO_A_NEW_ONLY_LOAD" "$TASK_B_DATASET" "$OUTPUT_ROOT/b_to_a_new_only_masked.json" "$SOURCE_NUM_EXPERTS" "$((MASTER_PORT + 9))"
