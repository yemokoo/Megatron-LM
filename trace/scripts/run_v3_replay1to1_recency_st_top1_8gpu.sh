#!/bin/bash
# v3_new at 1:1 replay volume with a recency-weighted budget split.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_replay1to1_recency}"
OUTPUT="${RUN_ROOT}/v3_new_replay1to1_recency_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29781}"
...
# Same 5,000 budget, tilted toward recent tasks: weight k**power at recency
# rank k, so round 8 gives NumGLUE-ds 1,250 and C-STANCE 179 instead of 714
# each.  v3_new loses 9.85 on the former and 0.35 on the latter, so equal
# shares are spending most of the budget where nothing decays.
export OURS_REPLAY_DISTRIBUTION=recency_weighted
export OURS_REPLAY_RECENCY_POWER="${OURS_REPLAY_RECENCY_POWER:-1.0}"

echo "[TRAIN START] v3_new_replay1to1_recency  GPUs=${OURS_LORAMOE_GPUS}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_replay1to1_recency
