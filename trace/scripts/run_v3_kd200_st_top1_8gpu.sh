#!/bin/bash
# v3_replay1to1 with KD-init doubled again, to find where KD saturates.
#
# Raising KD from 20% to 100% of a primary epoch cut forgetting from 2.78 to
# 0.74 while barely moving AA (+0.24).  Nothing says 100% is the floor.  KD
# re-reads the same 500 stored records per task, so this costs compute only --
# the stored-memory figure the method claims is untouched.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_kd200}"
OUTPUT="${RUN_ROOT}/v3_new_kd200_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29861}"
...
echo "[TRAIN START] v3_new_kd200  GPUs=${OURS_LORAMOE_GPUS}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_kd200
