...
RESUME_CHECKPOINT="${RUN_DIR}/1"
LOG_DIR="${RUN_DIR}/eval_queue_logs"

if [[ ! -s "${RESUME_CHECKPOINT}/lora_moe_meta.json" ]]; then
  echo "[ERROR] missing FOMC checkpoint: ${RESUME_CHECKPOINT}" >&2
  exit 2
fi
mkdir -p "${LOG_DIR}"
...
echo "[RESUME CHAIN START] $(date '+%F %T %Z')"
echo "[TRAIN RESUME] checkpoint=${RESUME_CHECKPOINT} next=MeetingBank"
echo "[TRAIN CONFIG] gpus=0-7 micro=8 accum=1 global=64"
bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  train llama31 v1_expert_first

if [[ ! -s "${RUN_DIR}/7/lora_moe_meta.json" ]]; then
  echo "[ERROR] resumed training returned without final checkpoint" >&2
  exit 1
fi
...
echo "[EVAL START] $(date '+%F %T %Z') sparse-15 gpus=0-7"
...
echo "[RESUME CHAIN COMPLETE] $(date '+%F %T %Z') run_dir=${RUN_DIR}"
