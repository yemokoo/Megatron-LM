#!/usr/bin/env python3
# Fork of run_ours_sparse15_optimized.py that can drive any evaluator with
# the same CLI. evaluate_tab1.py reuses evaluate_Ours_LoRA_MoE.parse_args,
# so the Table-1 rows (seq_lora, ewc, olora, mtl, lifelong_moe, moe_lpr)
# inherit this scheduler's per-task batch table and sample sharding
# unchanged. The original file is untouched so every Ours run keeps its
# provenance.
...
# Set once from --evaluator before any job is built; the command builder
# is called from several places and threading it through all of them would
# only add noise.
EVALUATOR = ["evaluate_Ours_LoRA_MoE.py"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--evaluator", default=os.environ.get(
            "SPARSE15_EVALUATOR", "evaluate_Ours_LoRA_MoE.py"),
        help="Evaluator entry point, resolved inside the llmcl_benchmark "
             "working directory. Use evaluate_tab1.py for the Table-1 rows.")
...
def main() -> int:
    ...
    for round_id in {cell.round_id for cell in cells}:
        # tab1_meta.json is written by every Table-1 row; lora_moe_meta.json
        # is a compatibility file some MoE layouts skip on purpose (a shared
        # LoRA adapter carries `.lora.` keys the Ours evaluator would reject,
        # see tab1_moe.py's write_meta/_has_shared_adapter) -- checking for it
        # here falsely treats a complete Lifelong-MoE checkpoint as missing.
        checkpoint = run_dir / str(round_id - 1) / "tab1_meta.json"
