#!/bin/bash
# v3_replay1to1 with the replay budget tilted toward recent tasks.
#
# The earlier recency run scored FM 2.22 against 3.14 for its equal-split
# counterpart, but both ran with KD cut to 7,000 exposures.  This repeats it
# at replay1to1's KD so the distribution is the only difference.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_recency_kd175k}"
OUTPUT="${RUN_ROOT}/v3_new_recency_kd175k_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29831}"
...
echo "[TRAIN START] v3_new_recency_kd175k  GPUs=${OURS_LORAMOE_GPUS}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_recency_kd175k
