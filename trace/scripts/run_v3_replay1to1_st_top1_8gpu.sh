#!/bin/bash
# v3_new with joint router replay raised to 1:1 against the primary stream.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_replay1to1}"
OUTPUT="${RUN_ROOT}/v3_new_replay1to1_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29761}"
...
echo "[TRAIN START] v3_new_replay1to1  GPUs=${OURS_LORAMOE_GPUS}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_replay1to1
