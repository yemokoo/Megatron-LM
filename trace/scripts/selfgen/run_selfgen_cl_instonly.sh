#!/usr/bin/env bash
# Self-generated-replay CL, FROZEN generation + INSTRUCTION-ONLY replay loss.
#
# Fork of run_selfgen_cl_frozen.sh with one change: the trainer is
# train_selfgen_instonly.py, which masks the generated ANSWER tokens out of the
# replay causal-LM loss (labels -100 from answer_start), so the router-only replay
# phase corrects routing on the instruction span only.  Stage B (answers) is kept so
# the records and KD-init are identical in kind to the frozen run; only the loss
# mask differs.  Round 0 (no replay) is identical to the frozen run and is seeded
# from it (model/0, its persistent memory json, gen/round_1/C-STANCE).
#
...
RUN=${RUN:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/selfgen_cl_instonly_20260901}
...
train_round() {   # t
  ...
  SELFGEN_ROOT="$GEN/round_$t" SELFGEN_CURRENT_TASK="$task" CUDA_VISIBLE_DEVICES=$GPU_LIST \
    ...
    $PY -m torch.distributed.run --nproc_per_node=$NGPU --master_port=${PORT:-29802} \
    ...
    --num_train_epochs 5,3,7,5,3,5,5,7 --per_device_train_batch_size ${PDB:-8} --gradient_accumulation_steps 1 \
...
if [ ! -f "$OUT/0/lora_moe_meta.json" ]; then
  ...
  cp -r "$FROZEN/model/0" "$OUT/0" && mkdir -p "$OUT/fixed_replay_memory" "$GEN/round_1/C-STANCE" \
    ...
    && ln -sf "$FROZEN/gen/round_2/C-STANCE/records.jsonl" "$GEN/round_1/C-STANCE/records.jsonl" \
