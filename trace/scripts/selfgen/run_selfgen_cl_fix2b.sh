#!/usr/bin/env bash
# Self-generated-replay CL over the 8 TRACE tasks -- fixed generation (fix2b).
# Same as run_selfgen_cl_fix2.sh but per-device batch 8 (global 64) from round 2 on;
# rounds 0-1 were trained at global 32.  Batch contract keys are waived on resume.
#
...
train_round() {   # t
  ...
  SELFGEN_ROOT="$GEN/round_$t" SELFGEN_CURRENT_TASK="$task" CUDA_VISIBLE_DEVICES=$GPU_LIST \
    RESUME_CONTRACT_ALLOW_DRIFT=active_stream_samples_per_primary_epoch,joint_replay_active_stream_samples_per_primary_epoch,memory_batch_size,kd_memory_batch_size,effective_kd_memory_batch_size,effective_replay_memory_batch_size,replay_forward_batch_size \
    $PY -m torch.distributed.run --nproc_per_node=$NGPU --master_port=29797 \
