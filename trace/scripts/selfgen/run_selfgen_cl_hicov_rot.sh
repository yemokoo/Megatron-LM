...
RUN=${RUN:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/selfgen_cl_hicov_rot_20260902}
...
train_round() {   # t
  ...
  SELFGEN_ROOT="$GEN/round_$t" SELFGEN_CURRENT_TASK="$task" CUDA_VISIBLE_DEVICES=$GPU_LIST \
    ...
    $PY -m torch.distributed.run --nproc_per_node=$NGPU --master_port=${PORT:-29807} \
