...
# Cards this run may use.  Rounds 0-5 ran on all 8 (global batch 8x8=64); from round 6
# the user freed two cards, so training is 8-way again (global batch 64) from round 7; the waiver below covers
# the cap drift recorded in the round-6 checkpoint (4998 -> 5000)
# shards over the same six.
...
train_round() {   # t
  ...
  SELFGEN_ROOT="$GEN/round_$t" SELFGEN_CURRENT_TASK="$task" CUDA_VISIBLE_DEVICES=$GPU_LIST \
    ...
    $PY -m torch.distributed.run --nproc_per_node=$NGPU --master_port=29781 \
...
gen_for_round() {   # produce G for the NEXT round: tasks 0..t generated from model_t
  ...
  mapfile -t plan < <($PY "$TRACE/scripts/selfgen/plan_gen_gpus.py" "$NGPU" "${spec[@]}")
