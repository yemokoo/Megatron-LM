#!/bin/bash
# v3_replay1to1 storing the whole task instead of a 10% subset.
#
# The earlier p5k run also had KD cut to 7,000, so its 61.65/3.14 says
# nothing about storage on its own.  Here only the stored memory moves.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_p5k_kd175k}"
OUTPUT="${RUN_ROOT}/v3_new_p5k_kd175k_st_top1"
...
echo "[TRAIN START] v3_new_p5k_kd175k  GPUs=${OURS_LORAMOE_GPUS}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_p5k_kd175k
