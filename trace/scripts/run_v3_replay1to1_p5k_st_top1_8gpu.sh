#!/bin/bash
# v3_new at 1:1 replay volume with the full task stored as memory.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_replay1to1_p5k}"
OUTPUT="${RUN_ROOT}/v3_new_replay1to1_p5k_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29801}"
...
echo "[TRAIN START] v3_new_replay1to1_p5k  GPUs=${OURS_LORAMOE_GPUS}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_replay1to1_p5k
