...
from train_bos_token import install_last_bias           # noqa: E402
...
def main():
    ...
    print(f"[gen] cond={cond} prompt_ids={prompt_ids} n={a.num_seqs}", flush=True)
    ...
    for b0 in range(0, a.num_seqs, a.batch):
        ...
        outp = model.generate(input_ids=ids, attention_mask=torch.ones_like(ids), do_sample=True,
                              ...
                              stopping_criteria=StoppingCriteriaList([EotCount(eot, a.eot_stop, L)]), use_cache=True)
        for r in range(b):
            g = outp[r, L:].tolist()
    ...
    stats = {"cond": cond, "prompt_ids": prompt_ids, "num": len(rows),
