...
#            all layers (other slots -inf; every other position routes freely), 1500 or 2000 docs/task spread over
#            8 GPUs, strict/lenient filter -> up to 1000 records; train task t on them; discard; repeat.
...
R="${TRACE_RUN_ROOT:-$TRACE/results/header_forced_v1_1500_2000_rep1000}"
ANCHORS="$TRACE/scripts/selfgen/assets/anchors.json"
export TRACE_DATA_ROOT="$DATA"
DEC=none
GEN_PER_TASK=1500          # default requested count; MeetingBank/ScienceQA request 2000
SELECT_CAP=1000
...
task_request(){ case "$1" in MeetingBank|ScienceQA) echo 2000;; *) echo "$GEN_PER_TASK";; esac; }
...
train_round(){   # t
  ...
  ( cd "$IMPL" && env SELFGEN_ROOT="$R/gen/round_$t" SELFGEN_CURRENT_TASK="$task" CUDA_VISIBLE_DEVICES=0,1,2,3,4,5,6,7 \
      ...
      RESUME_CONTRACT_ALLOW_DRIFT=active_stream_samples_per_primary_epoch,joint_replay_active_stream_samples_per_primary_epoch,persistent_samples_per_task \
      ...
      --v2_new_active_memory_cap 5000 --v2_new_persistent_samples_per_task 1000 --v2_new_active_unique_cap 7000 \
...
gen_round(){   # t : regenerate tasks 0..t-1 from model/(t-1) into gen/round_t, 8 GPUs x (task chunks)
  ...
  $PY "$TRACE/scripts/bos_token/regen_queue_v1.py" --checkpoint "$ckpt" --dest "$dest" \
    ...
    --default-per-task "$GEN_PER_TASK" --meetingbank-per-task 2000 --scienceqa-per-task 2000 --gpus 0,1,2,3,4,5,6,7 \
    --guard-decision "$DEC" > "$dest/regen_queue.log" 2>&1 || { say "EVENT: STEP_FAILED regen round $t"; exit 1; }
  for j in $(seq 0 $((t-1))); do
    task=${TASKS[$j]}; target=$(task_request "$task")
    [ -f "$dest/$task/records.jsonl" ] && continue
    cat "$dest/$task"/records.part*.jsonl > "$dest/$task/records.all.jsonl"
    $PY - "$dest/$task" "$j" "$ANCHORS" "$target" "$SELECT_CAP" <<'PYEOF' > "$dest/$task/select.log" 2>&1
import json, re, sys, glob, random
d, j, anchors_path, req, select_cap = sys.argv[1], sys.argv[2], sys.argv[3], int(sys.argv[4]), int(sys.argv[5])
...
random.Random(0).shuffle(keep); keep = keep[:select_cap]
...
PYEOF
  ...
  for k in 0 1 2 3 4 5 6 7; do
    per=$((GEN_PER_TASK / 8 + (k < GEN_PER_TASK % 8)))
...
if [ ! -f "$R/model/sparse15_summary.json" ]; then
  ...
  ( cd "$TRACE" && SPARSE15_EVAL_EXTRA_ARGS="--bos_guard --guard_header --guard_decision $DEC" \
      ...
      --run-dir "$R/model" --method header_forced_v1_1500_2000_rep1000 --gpus 0,1,2,3,4,5,6,7 ) > "$R/logs/eval.log" 2>&1
