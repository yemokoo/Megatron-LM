#!/bin/bash
# SLoRA-Pre plus the replay data v3 uses -- the compute-matched control.
#
# v3 beats SLoRA-Pre (63.51/0.74 against 60.84/3.58) while also replaying
# 5,000 old-task records per epoch that SLoRA-Pre never sees.  That leaves an
# unanswered question: is the margin the router-only LoRA-MoE mechanism, or is
# it simply the extra data?  This run gives SLoRA-Pre the *same* replay -- the
# same stored indices, the same per-round allocation, read straight out of the
# v3 run's fixed_replay_memory/ and replay_plans/ -- and changes nothing else.
#
# The one intended difference is where the replay gradient goes:
#   v3    replay grad -> routers only, experts frozen
#   here  replay grad -> the whole new LoRA (q,k,v,o,gate,up,down)
#
# Structure is matched too: one replay micro-batch per primary micro-batch,
# loss = LM(primary) + 1.0 * LM(replay) in a single optimizer update.  So the
# step count stays at the released 313/epoch and neither optimizer steps nor
# replay volume can explain a difference.
#
# Reading it:
#   still loses to v3   -> the margin is the mechanism, not the data
#   catches up to v3    -> the margin was the data, and v3's claim narrows
#   beats its own base  -> replay helps SLoRA too, quantifies how much
#
# Batch: unchanged from the reproduction, 4 x 4 x 1 = 16 effective.
set -euo pipefail
...
export SLORA_OUTPUT_ROOT="${SLORA_OUTPUT_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/slora_pre_replay}"

# The replay source: a completed v3 run whose stored memory and per-round
# plans this control reuses verbatim.  replay_memory.py re-checks each
# index list against the artifact's own sha256 before using it.
export SLORA_REPLAY_V3_RUN_DIR="${SLORA_REPLAY_V3_RUN_DIR:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_replay1to1/v3_new_replay1to1_st_top1}"
export SLORA_REPLAY_LOSS_COEFF="${SLORA_REPLAY_LOSS_COEFF:-1.0}"
for required in replay_plans fixed_replay_memory; do
  [[ -d "${SLORA_REPLAY_V3_RUN_DIR}/${required}" ]] || {
    echo "[ERROR] ${SLORA_REPLAY_V3_RUN_DIR} has no ${required}/" >&2; exit 2; }
done
...
echo "[SLORA] replay=${SLORA_REPLAY_V3_RUN_DIR} coeff=${SLORA_REPLAY_LOSS_COEFF}"
