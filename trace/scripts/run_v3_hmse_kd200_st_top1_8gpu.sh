#!/bin/bash
# hidden-MSE replay with KD-init doubled, the one setting left that could make
# that objective competitive.
#
# hidden-MSE loses to LM at matched settings (60.64/4.94 against 63.51/0.74),
# but it is unusually sensitive to KD-init: its teacher is the model as it stood
# right after KD-init, so a weak KD phase leaves it anchoring to a weak
# reference.  Dropping KD to 7k took it from 60.64 to 50.59, five times the
# swing the same change costs LM.  If doubling KD does not close the gap,
# hidden-MSE is finished as a replay objective here.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_hmse_kd200}"
OUTPUT="${RUN_ROOT}/v3_new_hmse_kd200_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29881}"
...
export OURS_V2_KD_EXPOSURE_SAMPLES=10000


echo "[TRAIN START] v3_new_hmse_kd200  GPUs=${OURS_LORAMOE_GPUS}"
echo "[TRAIN START] base model = ${SLORA_LLAMA31_PATH}"
echo "[TRAIN START] replay 5,000/epoch (1:1), objective=${OURS_V2_JOINT_REPLAY_OBJECTIVE}, distribution=${OURS_REPLAY_DISTRIBUTION:-equal_task}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_hmse_kd200
