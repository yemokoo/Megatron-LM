#!/bin/bash
# hidden-MSE joint replay at the same 1:1 volume as v3_new_replay1to1.
#
# hidden-MSE lost badly at 5:1 (OP 54.63 vs 62.08) but preserved wiki and code
# on the a100 chain, where it was fitted against far more replay per router
# parameter.  Pairing it with the LM run at identical volume separates the
# objective from the amount.
#
...
RUN_ROOT="${OURS_ST_RUN_ROOT:-/data2/seonghyeonnoh/LLM-continual-learning-runs/trace/v3_hidden_mse_1to1}"
OUTPUT="${RUN_ROOT}/v3_new_hidden_mse_1to1_st_top1"
...
export OURS_LORAMOE_PORT="${OURS_LORAMOE_PORT:-29771}"
...
echo "[TRAIN START] v3_new_hidden_mse_1to1  GPUs=${OURS_LORAMOE_GPUS}"
echo "[TRAIN START] base model = ${SLORA_LLAMA31_PATH}"
echo "[TRAIN START] replay 5,000/epoch (1:1), objective=hidden_mse, distribution=${OURS_REPLAY_DISTRIBUTION:-equal_task}"
...
exec bash "${ROOT}/scripts/baselines/_run_ours_lora_moe.sh" \
  "${OURS_ACTION:-train}" llama31 v3_new_hidden_mse_1to1
