...
class GEM(CL_Base_Model):
    ...
    def project2cone2(self, name, gradient, indx, margin=0.5, eps=1e-3):
        ...
        memories = self.grads[name].to(gradient.device).index_select(1, indx)
        ...
        #     # gradient.copy_(torch.Tensor(x).view(-1, 1))
            ...
            if self.args.implementation_variant == "corrected":
                ...
                h = torch.full((t,), -margin, device=gradient.device)
            else:
                q = -q
                G = torch.eye(t, device=gradient.device)
                h = torch.full((t,), margin, device=gradient.device)
            e = torch.empty(0, device=gradient.device)
