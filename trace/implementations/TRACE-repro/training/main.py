...
# The released monkey patch targets an old Llama attention API and breaks
# Llama 3.1 and Qwen2.5. Modern Transformers selects native SDPA.
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help="Warmup fraction; used when num_warmup_steps is zero.")
    ...
    parser.add_argument(
        '--implementation_variant', choices=['upstream', 'corrected'],
        default='upstream',
        help='Published implementation or audited correction.')
...
def main():
    ...
    if args.CL_method == "O-LoRA":
        ...
        peft_config = LoraConfig(
            task_type=TaskType.CAUSAL_LM, r=64, lora_alpha=128, lora_dropout=0.1,
    ...
    def get_optimizer(model):
        ...
        total_micro_batches = sum(
            len(train_task_list[task]) * int(args.num_train_epochs[index])
            for index, task in enumerate(train_task_list)
        )
        total_update_steps = math.ceil(
            total_micro_batches / args.gradient_accumulation_steps)
        warmup_steps = args.num_warmup_steps or math.ceil(
            total_update_steps * args.warmup_ratio)
