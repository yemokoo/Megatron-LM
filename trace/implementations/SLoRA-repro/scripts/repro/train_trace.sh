...
LORA_DROPOUT="${SLORA_LORA_DROPOUT:-0.05}"
SKIP_COMPLETED="${SLORA_SKIP_COMPLETED:-0}"
CUDA_VISIBLE_DEVICES="${CUDA_VISIBLE_DEVICES:-0,1,2,3}"
LLAMA31_TOKEN_CACHE="${SLORA_LLAMA31_TOKEN_CACHE:-${SCAFFOLD}/cache/tokenized/llama31_8b/slora_chat_full_len1024}"
...
case "${METHOD}" in
  ...
  *)
  ...
  if [[ "${METHOD}" == "seq" && "${MODEL_KEY}" == "llama31" ]]; then
    [[ -f "${LLAMA31_TOKEN_CACHE}/manifest.json" ]] || {
      echo "[ERROR] Llama token cache missing: ${LLAMA31_TOKEN_CACHE}" >&2
      exit 4
    }
  fi
if [[ "${DRY_RUN}" != "1" ]]; then
...
for index in "${!TASKS[@]}"; do
  ...
  if [[ "${SKIP_COMPLETED}" == "1" \
        && -s "${task_output}/adapter_config.json" \
        && -s "${task_output}/adapter_model.safetensors" ]]; then
    echo "[SKIP] ${task} order${order}: completed adapter exists"
    continue
  fi
  ...
  if [[ "${METHOD}" == "seq" && "${MODEL_KEY}" == "llama31" ]]; then
    command+=(--tokenized_train_cache_dir "${LLAMA31_TOKEN_CACHE}")
  fi
  if [[ "${METHOD}" == "pre" ]]; then
    command+=(--mode max)
  fi
