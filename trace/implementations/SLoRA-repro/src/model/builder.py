...
def load_continual_pretrained_model(model_path, model_base, model_name, denoised_mode, load_8bit=False, load_4bit=False, use_logit_bias=False, device_map="auto", device="cuda", test_order=0, denoised_num=0):
    ...
    if model_base is not None:
        tokenizer = AutoTokenizer.from_pretrained(model_base, use_fast=False, trust_remote_code=True, local_files_only=True)
        model = AutoModelForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, trust_remote_code=True, local_files_only=True, **kwargs)
        base_model = model
        reference_weights = (
            snapshot_reference_weights(base_model)
            if denoised_mode == "slora_post"
            else None
        )
        ...
        for idx, item_path in enumerate(task_paths):
            ...
            if denoised_mode=='slora_pre':
                ...
                if denoised_lora_path and not os.path.exists(denoised_lora_path):
                    ...
                    new_state_dict = denoising(reference_weights, state_dict)
