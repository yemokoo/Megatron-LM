...
from src.trace_data import prompt_answer
    ...
    def __getitem__(self, index):
        ...
        for i in range(index*bz, (index+1)*bz):
            if i < self.size:
                ...
                question, answer = prompt_answer(line)
...
def eval_model(args):
    ...
    if tokenizer_with_prefix_space is not None:
        ...
        with open(answers_file, 'r', encoding='utf-8') as f:
            for line_number, line in enumerate(f, 1):
                try:
                    ...
                    raise ValueError(
                        f"Cannot resume from malformed JSONL line {line_number}: {answers_file}"
                    ) from exc
                missing = {"prompt", "text", "solution"} - record.keys()
                if missing:
                    raise ValueError(
                        f"Cannot resume: line {line_number} is missing {sorted(missing)}"
                    )
                if line_number > len(questions):
                    raise ValueError(
                        f"Cannot resume: outputs exceed {len(questions)} questions"
                    )
                source = questions[line_number - 1]
                _, expected_solution = prompt_answer(source)
                if record["solution"] != expected_solution:
                    raise ValueError(
                        f"Cannot resume: line {line_number} does not match "
                        "the source-question prefix"
                    )
                current_file_num += 1
        if current_file_num > len(questions):
            raise ValueError(
                f"Cannot resume: {current_file_num} outputs exceed {len(questions)} questions"
            )
        print(f"Validated {current_file_num}/{len(questions)} existing outputs", flush=True)
    ...
    def generate_with_oom_fallback(batch_prompts):
        input_ids = tokenizer(
            batch_prompts, return_tensors='pt', padding=True
        ).input_ids.to(device='cuda', non_blocking=True)
        attention_mask = input_ids.ne(tokenizer.pad_token_id).to(
        ...
        try:
            ...
            if len(batch_prompts) == 1:
                raise
            split_at = len(batch_prompts) // 2
            print(
                f"[OOM fallback] retrying batch of {len(batch_prompts)} "
                f"as {split_at}+{len(batch_prompts) - split_at}",
                flush=True,
            )
        # Retry only after leaving the except suite: its traceback can retain
        # tens of GiB of partially-built attention tensors.
        del input_ids, attention_mask
        gc.collect()
        torch.cuda.empty_cache()
        return (
            generate_with_oom_fallback(batch_prompts[:split_at])
            + generate_with_oom_fallback(batch_prompts[split_at:])
        )
    ...
    for idx in trange(len(dataset)):
        ...
        outputs = generate_with_oom_fallback(prompts)
