...
R=64
ALPHA=128
CKPT=llama3-8b-lora-r${R}a${ALPHA}
...
for idx in "${!TRAINING_DATA[@]}"; do
    ...
    if [ -f "$TEMP_OUTPUT_DIR/$MODE.safetensors" ]; then
    ...
    bash ./scripts/train/my_slurm/slora_pre.sh $MODEL_BASE $MODEL "$TASK_NAME" "$TASK_FILE" $TEMP_OUTPUT_DIR $ORDER $TRAIN_EPOCH $MODE $R $ALPHA  &> $LOG_FILE 
